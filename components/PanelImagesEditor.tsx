...
interface Props {
  images: PanelImage[];
  /** Fungsi pembaharu, bukan array jadi: lihat alasan yang sama di DecorationsEditor (dua unggahan hampir
   * bersamaan tidak boleh saling menimpa karena salah satunya dihitung dari array yang sudah basi). */
  onChange: (updater: (prev: PanelImage[]) => PanelImage[], key: string) => void;
}
...
export function PanelImagesEditor({ images, onChange }: Props) {
  ...
  const patch = (id: string, fn: (i: PanelImage) => PanelImage) => onChange((prev) => prev.map((i) => (i.id === id ? fn(i) : i)), `panel.image.${id}`);
      ...
      <div className="flex flex-wrap gap-2">
        {(["behind", "front"] as const).map((layer) => (
          <button
            ...
            onClick={() => onChange((prev) => [...prev, newPanelImage(layer)], `panel.image.add.${layer}`)}
          ...
          {images.map((img, index) => (
            <li key={img.id} className="grid gap-4 rounded-field border border-line bg-surface-2/50 p-3">
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  type="button"
                  onClick={() => onChange((prev) => prev.filter((i) => i.id !== img.id), `panel.image.remove.${img.id}`)}
              ...
              <CanvasEditButton imgRef={{ kind: "panel", id: img.id }} disabled={img.url === ""} />
              <Segmented
                label="Layer"
                small
                value={img.layer}
              ...
              <SliderField label="Offset X" value={img.offsetX} min={-600} max={600} unit="px" onChange={(offsetX) => patch(img.id, (i) => ({ ...i, offsetX }))} />
              <SliderField label="Offset Y" value={img.offsetY} min={-600} max={600} unit="px" onChange={(offsetY) => patch(img.id, (i) => ({ ...i, offsetY }))} />
